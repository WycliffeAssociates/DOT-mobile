...
export function makeVidSaver(
  ...
  return {
    ...
    async calculateNeededRangesToFetch(fileSize: number) {
      ...
      let inProgressData = inProgressStringified?.value
      ...
      if (inProgressData) {
        ...
        for (let i = 0; i < numberOfRangeFetches; i++) {
          let possibleEnd = i * increment + increment;
          let end = possibleEnd > fileSize ? fileSize : possibleEnd;
          let rangeRequestParams = {
    ...
    async getCapacitorSrcForFinalBlob() {
      ...
      let mp4DeviceSrc = Capacitor.convertFileSrc(finalMp4Blob.uri);
      return mp4DeviceSrc;
    },
    ...
    async deleteAllVidData(playlistData: IPlaylistData, vid: IVidWithCustom) {
      ...
      try {
        ...
        delete currentVidInThatClone.savedSources;
